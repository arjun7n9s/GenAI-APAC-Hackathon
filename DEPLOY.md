...
## Refresh the repo in Cloud Shell

If Cloud Shell already has an older checkout, replace it completely with the latest GitHub state:

```bash
cd "$HOME"
rm -rf productivity_multi_agent
git clone https://github.com/arjun7n9s/GenAI-APAC-Hackathon.git productivity_multi_agent
cd productivity_multi_agent
git status
```

If you want to keep the existing folder and hard-reset it to the latest remote state instead:

```bash
cd "$HOME/productivity_multi_agent"
git fetch origin
git checkout main
...
Then set the deployment variables:
